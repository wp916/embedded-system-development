...
TickType_t usmart_old_time = 0;
	
//复位runtime
//需要根据所移植到的MCU的定时器参数进行修改
void usmart_reset_runtime(void)
{
	usmart_old_time = xTaskGetTickCountFromISR();
}
//获得runtime时间
...
//	}
	TickType_t usmart_new_time=xTaskGetTickCountFromISR();
	usmart_dev.runtime= (uint32_t) ((usmart_new_time - usmart_old_time) * portTICK_PERIOD_MS);
...
//uint8_t USART_RX_BUF[200];     //接收缓冲,最200个字节.
...
void usmart_scan(uint8_t* data, uint16_t len)
{
	uint8_t sta;  
...
//	sta=usmart_dev.cmd_rec(USART_RX_BUF);//得到函数各个信息
	sta=usmart_dev.cmd_rec(data);//得到函数各个信息
	if(sta==0)usmart_dev.exe();	//执行函数 
	else 
	{  
//		len=usmart_sys_cmd_exe(USART_RX_BUF);
		len=usmart_sys_cmd_exe(data);
